...
class ListUser
  ...
  require 'workloadholiday_datetools'
  ...
  def self.getOpenIssuesForUsers(users)
    ...
    issues = Issue.joins(:project).
        ...
        where("#{Issue.table_name}.project_id <> ?", Setting['plugin_redmine_workloadholiday']['holidays_project']).# Do not belong to holidays project
        where(project[:status].eq(1)).# Do not belong to an inactive project
        where(issue_status[:is_closed].eq(false)) # Is open

    #ActionController::Base::logger.info issues.inspect
  ...
  def self.getHoursForIssuesPerDay(issue, timeSpan, today)
    ...
    workingDays = DateTools::getWorkingDaysInTimespan(timeSpan, issue.assigned_to)
    holidayDays = DateTools::getHolydayDaysInTimespan(timeSpan, issue.assigned_to)
    ...
    if !issue.due_date.nil? && (issue.due_date < today) then
      ...
      timeSpan.each do |day|
        ...
        result[day] = {
            ...
            :realholiday => holidayDays.include?(day)
        }
      end

      firstWorkingDayAfterToday = DateTools::getWorkingDaysInTimespan(today..timeSpan.end, issue.assigned_to).min
      ...
      timeSpan.each do |day|
        ...
        isRealHoliday = holidayDays.include?(day)
      ...
      numberOfWorkdaysForIssue = DateTools::getRealDistanceInDays([today, issue.start_date].max..issue.due_date, issue.assigned_to)
  ...
  #								for.
  def self.getHoursPerUserIssueAndDay(users, issues, timeSpan, today)
    raise ArgumentError unless users.kind_of?(Array)
    raise ArgumentError unless issues.kind_of?(Array)
    raise ArgumentError unless timeSpan.kind_of?(Range)
    raise ArgumentError unless today.kind_of?(Date)

    workingDaysByUser = DateTools::getWorkingDaysForUsersInTimespan(timeSpan, users)
    holydayDaysByUser = DateTools::getHolydayDaysForUsersInTimespan(timeSpan, users)
    ...
    issues.each do |issue|
      ...
      if !result.has_key?(issue.assigned_to) then
        ...
        timeSpan.each do |day|
          result[assignee][:total][day] = {
              :hours => 0.0,
              :holiday => !workingDaysByUser[assignee].include?(day),
              :realholiday => holydayDaysByUser[assignee].include?(day)
          }
        end
      end
      ...
      if issue.overdue? then
        ...
        result[assignee][:total] = addIssueInfoToSummary(result[assignee][:total], hoursForIssue, timeSpan, assignee)
      end
      ...
      if !issue.visible? then
        result[assignee][:invisible] = addIssueInfoToSummary(result[assignee][:invisible], hoursForIssue, timeSpan, assignee) unless issue.overdue?
      else
        project = issue.project

        #ActionController::Base::logger.info project.inspect
        ...
        if issue.overdue? then
          ...
          result[assignee][project][:total] = addIssueInfoToSummary(result[assignee][project][:total], hoursForIssue, timeSpan, assignee)
        end
  ...
  def self.getLoadClassForHours(hours)
    ...
    if hours < Setting['plugin_redmine_workloadholiday']['threshold_lowload_min'].to_f then
      return "none"
    elsif hours < Setting['plugin_redmine_workloadholiday']['threshold_normalload_min'].to_f then
      return "low"
    elsif hours < Setting['plugin_redmine_workloadholiday']['threshold_highload_min'].to_f then
      return "normal"
    else
      return "high"
    end
  ...
  def self.addIssueInfoToSummary(summary, issueInfo, timeSpan, user)
    workingDays = DateTools::getWorkingDaysInTimespan(timeSpan, user)
    holydayDays = DateTools::getHolydayDaysInTimespan(timeSpan, user)

    summary = Hash::new if summary.nil?

    timeSpan.each do |day|
      if !summary.has_key?(day) then
        summary[day] = {:hours => 0.0, :holiday => !workingDays.include?(day), :realholiday => holydayDays.include?(day)}
      end
